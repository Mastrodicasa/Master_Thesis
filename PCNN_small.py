...
# The patches are 10*10 with a stride of 10.
...
def cnn_model_fn(features, labels, mode, params):  
  ...
  # No padding.
  # Input Tensor Shape: [batch_size*nbr_patch, 10, 10, 1]
  # Output Tensor Shape: [batch_size*nbr_patch, 6, 6, 10]
  ...
  # Input Tensor Shape: [batch_size*nbr_patch, 6, 6, 10]
  # Output Tensor Shape: [batch_size*nbr_patch, 3, 3, 10]
  pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)

  

  # Flatten tensor into a batch of vectors
  # Input Tensor Shape: [batch_size*nbr_patch, 3, 3, 10]
  # Output Tensor Shape: [batch_size*nbr_patch, 3 * 3 * 10]
  pool2_flat = tf.reshape(pool1, [-1, 3 * 3 * 10])
  

  # Dense Layer
  # Densely connected layer with 300 neurons
  # Input Tensor Shape: [batch_size*nbr_patch, 3 * 3 * 30]
...
def main(unused_argv):
    #Load the data
    class1 = scipy.io.loadmat('allClass1.mat')
    class1=class1['allClass1']
    label1=np.ones(class1.shape[0])
    
    class2 = scipy.io.loadmat('allClass2.mat')
    ...
    size_patch=10
    all_patch_train, label_patch_train, nbr_patch=sw.sliding_window(x_train, y_train, 10, size_patch)
    all_patch_test, label_patch_test, nbr_patch=sw.sliding_window(x_test, y_test, 10, size_patch)
...
#       #sw.show_one_image(i, x_test)
    ...
    mnist_classifier = tf.estimator.Estimator(
      model_fn=cnn_model_fn, model_dir="/tmp/PCNN9_small", params= {"train":batch_train, "test":batch_test,
                                                                    "nbr_patch":nbr_patch, 
                                                                    "nbr_class":nbr_class,
